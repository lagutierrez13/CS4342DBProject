...
<head>
    ...
    <title>CS4342 Create Admin Account</title>
...
<body>
    <div style="margin-top: 20px" class="container">
        ...
        if (isset($_POST['Submit'])) {
            ...
            //Insert into employee and advisor table;

            $queryAdmin  = "INSERT INTO employee (Eid,Ename,Eemail,Epassword)
                        VALUES ('" . $eid . "', '" . $ename . "', '" . $eemail . "', '" . $epassword . "');";

            $queryAdmin2  = "INSERT INTO administrator (Eid)
                        VALUES ('" . $eid . "');";

            // The query sent to the DB can be printed by not commenting the following row
            //echo $queryStudent;
            if (($conn->query($queryAdmin) === TRUE) and ($conn->query($queryAdmin2) === TRUE)) {
                echo "<br> New record created successfully for advisor id " . $eid;
            } else {
                echo "<br> The record was not created, the query: <br>" . $queryAdmin . "  <br> Generated the error <br>" . $conn->error;
            }
